...
- [x] **Step 3: Store e hook de toast**
...
- [x] **Step 1: Mocks globais de Jest (determinísticos para todos os testes)**
...
- [x] **Step 4: Rodar, typecheck e commit**
...
- [x] **Step 1: Gerador da foto-exemplo (PNG puro em Node, sem dependências)**
...
- [x] **Step 3: Asset resolvido, fontes para Expo Go, rota de QA e layout raiz**
...
- [x] **Step 4: Verificar no Jest, no bundle e no Expo Go**
...
## Notas de execução (plano 1b-1 concluído)

- **lucide-react-native** publica só ESM (`.mjs`) no campo `exports`; no Jest ele é mapeado para o build CJS via `moduleNameMapper` (`jest.config.js`).
- RNTL 14 não tem `toHaveAccessibilityState`; usamos `toBeSelected()`. RN 0.86 removeu `StyleSheet.absoluteFillObject` do tipo — usar `StyleSheet.absoluteFill` num array de estilos.
- `createExportService({ timeoutMs })` ganhou parâmetro para o teste de timeout não depender de fake timers (o mock do Reanimated agenda rAF com timers reais).
- Mocks globais adicionados ao `jest.setup.ts`: `expo-image` (vira `Image` do RN, permite `fireEvent(el,'load')`), `react-native-view-shot`, `expo-file-system/legacy` (FS em memória), `expo-image-picker`, `expo-image-manipulator`, `expo-sharing`, `expo-media-library/legacy`, `expo-haptics`.
- `removeOlderThan` usa try/catch em `readDirectoryAsync` (diretório inexistente) em vez de checar `getInfoAsync` antes.
- Estado final: **21 suítes / 109 testes**, `tsc --noEmit` limpo, `expo export` com o bundle e a foto-exemplo.
